...
*   `data/`: 데이터 관련 파일이 들어 있습니다.
    ...
    *   `raw_data/`: 원시 비디오 데이터가 들어 있습니다.
*   `stable_model/`: 훈련된 모델 가중치 및 훈련 결과가 들어 있습니다.
    *   `best.pt`: 가장 성능이 좋은 모델 가중치입니다.
    *   `train/`: 훈련 로그, 메트릭 및 시각화가 들어 있습니다.
*   `requirements.txt`: 프로젝트 종속성을 나열합니다.
*   `yolov8n.pt`: 미리 훈련된 YOLOv8n 모델 가중치입니다.
...
2.  YOLOv8 모델을 훈련하려면 `YOLO_train.py` 스크립트를 실행하십시오.
    ...
    *   훈련된 모델 가중치 및 훈련 결과를 `stable_model/train/` 디렉토리에 저장합니다.
...
2.  비디오에서 객체 감지를 수행하려면 `YOLO_inference.py` 스크립트를 실행하십시오.
    ...
    ```
    이 스크립트는 다음을 수행합니다.
    *   `stable_model/best.pt`에서 훈련된 YOLOv8 모델 가중치를 로드합니다.
...
*   `data/`: 包含与数据相关的文件。
    ...
    *   `raw_data/`: 包含原始视频数据。
*   `stable_model/`: 包含训练的模型权重和训练结果。
    *   `best.pt`: 性能最佳的模型权重。
    *   `train/`: 包含训练日志、指标和可视化。
*   `requirements.txt`: 列出项目依赖项。
*   `yolov8n.pt`: 预训练的YOLOv8n模型权重。
...
2.  运行`YOLO_train.py`脚本以训练YOLOv8模型。
    ...
    *   使用指定的参数训练YOLOv8n模型。
    *   评估测试集上训练的模型。
    *   将训练的模型权重和训练结果保存在`stable_model/train/`目录中。
...
2.  运行`YOLO_inference.py`脚本以对视频执行对象检测。
    ...
    ```
    此脚本将：
    *   从`stable_model/best.pt`加载训练的YOLOv8模型权重。
    *   对指定的视频文件运行推理。
    *   显示检测到的对象周围带有边界框的结果。
